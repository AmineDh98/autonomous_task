...
class OnlinePlanner:
    ...
    def __init__(self, gridmap_topic, odom_topic, cmd_vel_topic, dominion, distance_threshold):
        ...
        self.imu_sub = rospy.Subscriber("/turtlebot/kobuki/sensors/imu", Imu, self.Imu_callback)   
    ...
    def controller(self, event):
        ...
        if self.path is not None and len(self.path) > 0:
            ...
            if len(self.path) > 1:
                ...
                if np.abs(wrap_angle(self.Imu_angle[2] - angle(self.current_pose[0:2], self.goal[0:2]))) >0.1:
                    v, w = move_to_point_goal(self.current_pose, self.path[0][0:2], self.Imu_angle) 
                else:
                    #self.goal = None
                    self.path = None
...
if __name__ == '__main__':
    ...
    node = OnlinePlanner('/projected_map', 'kobuki/odom', '/turtlebot/kobuki/commands/wheel_velocities', np.array([-15.0, 15.0]), 0.21)  
