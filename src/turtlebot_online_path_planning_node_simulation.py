...
class OnlinePlanner:
    ...
    def controller(self, event):
        ...
        if self.path is not None and len(self.path) > 0:
            ...
            if np.linalg.norm(self.path[0][0:2] - self.current_pose[0:2]) < 2*self.svc.resolution : # and np.linalg.norm(self.path[0][0:1] != self.goal): 
                ...
                v, w = velocity_control_with_interpolation(self.current_pose[0], self.current_pose[1], self.current_pose[2], self.path)
                
                #v, w = pure_pursuit_control(self.current_pose[0], self.current_pose[1], self.current_pose[2], self.path)
                # else:
...
if __name__ == '__main__':
    ...
    node = OnlinePlanner('/projected_map', 'kobuki/odom', '/turtlebot/kobuki/commands/wheel_velocities', np.array([-15.0, 15.0]), 0.2)  
