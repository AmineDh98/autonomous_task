    ...
    def explore(self, segment_points):
        ...
        while var1 and segment_points !=[]:
            ...
            for i in segment_points:
                ...
                np.abs(self.angle(points_to_compare, self.current_pose))
                index4 = index4+1
            ...
            for i in range(len(segment_points)):
                combine[i]= (number[i]/100)+ 10/(dist[i]) + 4/(angle[i])
            ...
            #self.frontier_publisher(segment_to_go)
            #self.frontier_publisher(segment_points[21])
