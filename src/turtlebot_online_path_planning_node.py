...
class OnlinePlanner:
    ...
    def controller(self, event):
        ...
        if self.path is not None and len(self.path) > 0:
            ...
            if np.linalg.norm(self.path[0][0:2] - self.current_pose[0:2]) < 3*self.svc.resolution: 
                ...
                v, w = compute_dynamic_window(self.current_pose, self.current_v, self.current_w, self.path, self.target_velocity, self.svc) 
                # else:
...
if __name__ == '__main__':
    ...
    node = OnlinePlanner('/projected_map', 'kobuki/odom', '/turtlebot/kobuki/commands/wheel_velocities', np.array([-15.0, 15.0]), 0.24)  
