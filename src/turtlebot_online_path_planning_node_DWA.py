...
class OnlinePlanner:
    ...
    def plan(self):
        ...
        while len(path) == 0 and trial < 10:
            ...
            if not self.svc.is_valid(self.current_pose[0:2]):
                ...
                angle_difference = np.abs(self.current_pose[2] - angle)
    ...
    def controller(self, event):
        ...
        if self.path is not None and len(self.path) > 0:
            ...
            if np.linalg.norm(self.path[0][0:2] - self.current_pose[0:2]) < 8*self.svc.resolution and np.linalg.norm(self.path[0][0:1] != self.goal[0:1]):
                print("Position {} reached".format(self.path[0]))
                print('cuurent orientation', self.current_pose[2])  
                del self.path[0] 
                # if len(self.path) == 0 :
                ...
                #     goal_reached_msg.data = self.goal_reached
                    ...
                    # # publish the message
                    # self.goal_reached_pub.publish(goal_reached_msg)
            elif np.linalg.norm(self.path[0][0:1] == self.goal[0:1]) and np.linalg.norm(self.path[0][0:2] - self.current_pose[0:2]) < 2*self.svc.resolution :
                del self.path[0] 
                if len(self.path) == 0 :
                    self.goal = None
                    rospy.set_param('goal', 1)  
                ...
                # if len(self.path) < 3: 
                #v, w = move_to_point(self.current_pose, self.path[0][0:2])  
                # else:
                #     v, w = move_to_point(self.current_pose, self.path[2][0:2]) 
                #v, w = calculate_velocities(self.current_pose, self.path)
                v, w = compute_dynamic_window(self.current_pose, self.current_v, self.current_w, self.path, self.svc) 
                # else:
                #     v, w = move_to_point(self.current_pose, self.path[5][0:2])     
                # w = ...
                self.goal_reached = False
                goal_reached_msg = Bool()
                goal_reached_msg.data = self.goal_reached
...
if __name__ == '__main__':
    ...
    node = OnlinePlanner('/projected_map', 'kobuki/odom', '/turtlebot/kobuki/commands/wheel_velocities', np.array([-15.0, 15.0]), 0.22)  
