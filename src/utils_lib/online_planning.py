...
class StateValidityChecker:
    ...
    # Constructor
    def __init__(self, distance=0.24, is_unknown_valid=True): ## 0.2, 0.3 for buffle  
...
def move_to_point(current, goal, Kv=0.5, Kw=0.5):
    ...
    if angle > 0.25 or angle < -0.25:
        v = 0
    return v, w
def compute_dynamic_window(current_pose, current_speed, current_yaw_rate, path, target_velocity, state_validity_checker):
    # Parameters for dynamic window approach
    max_speed = 0.5  # Maximum linear speed
    max_yaw_rate = 0.3  # Maximum angular speed
    max_accel = 1.0  # Maximum linear acceleration
    max_delta_yaw_rate = 0.3  # Maximum angular acceleration
    dt = 0.2  # Time step for simulation
    v_resolution = 0.01  # Linear velocity resolution
    yaw_rate_resolution = 0.01  # Angular velocity resolution
    v=current_speed
    w=current_yaw_rate
    ...
    for v in np.arange(v_min, v_max, v_resolution):
        for yaw_rate in np.arange(yaw_rate_min, yaw_rate_max, yaw_rate_resolution):
            ...
            cost = evaluate_trajectory(trajectory, path, target_velocity, state_validity_checker, current_pose)
...
def evaluate_trajectory( trajectory, path, target_velocity, state_validity_checker, current_pose):
  ...
  # Evaluate the trajectory using a cost function
    ...
    velocity_cost = np.abs(target_velocity - achieved_velocity)
