...
def compute_dynamic_window(current_pose, current_speed, current_yaw_rate, path,  state_validity_checker):
    ...
    max_speed = 1  # Maximum linear speed
    max_yaw_rate = 1.5  # Maximum angular speed
    max_accel = 1  # Maximum linear acceleration
    max_delta_yaw_rate = 0.6  # Maximum angular acceleration
    ...
    if current_yaw_rate< 0.8:
        
        yaw_rate_min = yaw_rate_min*2
        yaw_rate_max = yaw_rate_max+0.3
    ...
    for v in np.arange(v_min, v_max, v_resolution):
        for yaw_rate in np.arange(yaw_rate_min, yaw_rate_max, yaw_rate_resolution):
            ...
            # if clearance_cost!=1000 :
            #     continue
            cost = evaluate_trajectory(trajectory, path, max_speed, state_validity_checker, current_pose)
...
def generate_trajectory(v, yaw_rate, dt, current_pose):
    ...
    trajectory = []
    duration = 1.5  # Total duration of the trajectory
