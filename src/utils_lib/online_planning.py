...
class StateValidityChecker:
    ...
    # Constructor
    def __init__(self, distance=0.22, is_unknown_valid=True): ## 0.2, 0.3 for buffle  
...
def calculate_velocities(current_position, path):
    ...
    # if len(path) >6:
    #     target_position = path[6]
    # elif len(path) >5:
    #     target_position = path[5]
    # elif len(path) >4:
    #     target_position = path[4]
    # elif len(path) >3:
    #     target_position = path[3]
    # elif len(path) >2:
    #     target_position = path[2]
    # elif len(path) >1:
    ...
    if angle_to_target - current_position[2] > 1 or angle_to_target - current_position[2] < -1:
        v = 0
    return v, w
...
def compute_dynamic_window(current_pose, current_speed, current_yaw_rate, path,  state_validity_checker):
    ...
    max_yaw_rate = 1.5  # Maximum angular speed  # 1.5
    max_accel = 1 # Maximum linear acceleration #1
    max_delta_yaw_rate = 1  # Maximum angular acceleration 1
    #min_speed = -1
    dt = 0.2  # Time step for simulation
    #time was changed from 0.2 to 1 (it can affect a lot)
    v_resolution = 0.06  # Linear velocity resolution
    yaw_rate_resolution = 0.06  # Angular velocity resolution
    v=current_speed
    w=current_yaw_rate
