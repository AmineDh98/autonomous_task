...
def calculate_velocities(current_position, path):
    ...
    if angle_to_target - current_position[2] > 0.35 or angle_to_target - current_position[2] < -0.35:
        v = 0
    return v, w
...
def compute_dynamic_window(current_pose, current_speed, current_yaw_rate, path,  state_validity_checker):
    ...
    max_delta_yaw_rate = 0.33  # Maximum angular acceleration 0.6
    #min_speed = -1
    ...
    if np.abs(current_speed)<0.3 :
        v_min = v_min *1.5 
        v_max = v_max 
        
    # if current_yaw_rate< 0.5: #0.8
        
    #     yaw_rate_min = yaw_rate_min*1.5 #*2
