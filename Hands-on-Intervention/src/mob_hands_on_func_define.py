...
def W_DLS(A, damping):  
    ...
    '''  
    w = np.diag([1, 1, 1, 1, 1, 1])  # more weight less movement of the link         
    w_i = np.linalg.inv(w)     
...
def Jacobian(theta, ctheta, d, link): # theta is a list of four angles and link is from 1 to 4            
 
    J = np.eye(6)

    Final1 = np.array([[-d*math.sin(ctheta)*math.cos(theta[0,0]) - d*math.sin(theta[0,0])*math.cos(ctheta) + (-0.142*math.sin(theta[2,0]) + 0.1588*math.cos(theta[3,0]) + 0.0692)*(-math.sin(ctheta)*math.sin(theta[0,0]) + math.cos(ctheta)*math.cos(theta[0,0]))*math.cos(theta[1,0]) + (-0.142*math.sin(theta[2,0]) + 0.1588*math.cos(theta[3,0]) + 0.0692)*(-math.sin(ctheta)*math.cos(theta[0,0]) -math.sin(theta[0,0])*math.cos(ctheta))*math.sin(theta[1,0]) - 0.051*math.sin(ctheta)*math.cos(theta[0,0]) - 0.051*math.sin(theta[0,0])*math.cos(ctheta)],
                        [-d*math.sin(ctheta)*math.sin(theta[0,0]) + d*math.cos(ctheta)*math.cos(theta[0,0]) + (-0.142*math.sin(theta[2,0]) + 0.1588*math.cos(theta[3,0]) + 0.0692)*(-math.sin(ctheta)*math.sin(theta[0,0]) + math.cos(ctheta)*math.cos(theta[0,0]))*math.sin(theta[1,0]) + (-0.142*math.sin(theta[2,0]) + 0.1588*math.cos(theta[3,0]) + 0.0692)*(math.sin(ctheta)*math.cos(theta[0,0]) + math.sin(theta[0,0])*math.cos(ctheta))*math.cos(theta[1,0]) - 0.051*math.sin(ctheta)*math.sin(theta[0,0]) + 0.051*math.cos(ctheta)*math.cos(theta[0,0])],
                        [0],
                        [0],
                        [0],
                        [1]])
    Final2 = np.array([[-math.sin(ctheta)*math.sin(theta[0,0]) + math.cos(ctheta)*math.cos(theta[0,0]) ], 
                        [math.sin(ctheta)*math.cos(theta[0,0]) + math.sin(theta[0,0])*math.cos(ctheta)],
                        [0],
                        [0],
                        [0],
                        [0]])
    Final3 = np.array([[(-0.142*math.sin(theta[2,0]) + 0.1588*math.cos(theta[3,0]) + 0.0692)*(-math.sin(ctheta)*math.sin(theta[0,0]) + math.cos(ctheta)*math.cos(theta[0,0]))*math.cos(theta[1,0]) - (-0.142*math.sin(theta[2,0]) + 0.1588*math.cos(theta[3,0]) + 0.0692)*(math.sin(ctheta)*math.cos(theta[0,0]) + math.sin(theta[0,0])*math.cos(ctheta))*math.sin(theta[1,0])],
                        [(-0.142*math.sin(theta[2,0]) + 0.1588*math.cos(theta[3,0]) + 0.0692)*(math.sin(ctheta)*math.cos(theta[0,0]) + math.sin(theta[0,0])*math.cos(ctheta))*math.cos(theta[1,0]) - (-0.142*math.sin(theta[2,0]) + 0.1588*math.cos(theta[3,0]) + 0.0692)*(math.sin(ctheta)*math.sin(theta[0,0])- math.cos(ctheta)*math.cos(theta[0,0]))*math.sin(theta[1,0]) ], 
                        [0.142*math.sin(theta[2,0])],
                        [0],
                        [0],
                        [1]])
    Final4 = np.array([[-0.142*(math.sin(ctheta)*math.sin(theta[0,0])  + math.cos(ctheta)*math.cos(theta[0,0]))*math.sin(theta[1,0])*math.cos(theta[2,0]) - 0.142*( math.sin(ctheta)*math.cos(theta[0,0]) + math.sin(theta[0,0])*math.cos(ctheta))*math.cos(theta[1,0])*math.cos(theta[2,0])],
                        [-0.142*(math.sin(ctheta)*math.cos(theta[0,0]) + math.sin(theta[0,0])*math.cos(ctheta))*math.sin(theta[1,0])*math.cos(theta[2,0]) - 0.142*(math.sin(ctheta)*math.sin(theta[0,0]) - math.cos(ctheta)*math.cos(theta[0,0]))*math.cos(theta[1,0])*math.cos(theta[2,0])],
                        [0.142*math.sin(theta[2,0])],
                        [0],
                        [0],
                        [0]])
    Final5 = np.array([[-0.1588*(-math.sin(ctheta)*math.sin(theta[0,0]) + math.cos(ctheta)*math.cos(theta[0,0]))*math.sin(theta[1,0])*math.sin(theta[3,0]) - 0.1588*(math.sin(ctheta)*math.cos(theta[0,0]) + math.sin(theta[0,0])*math.cos(ctheta) )*math.sin(theta[3,0])*math.cos(theta[1,0])],
                        [-0.1588*(math.sin(ctheta)*math.cos(theta[0,0]) + math.sin(theta[0,0])*math.cos(ctheta))*math.sin(theta[1,0])*math.sin(theta[3,0]) - 0.1588*(math.sin(ctheta)*math.sin(theta[0,0]) - math.cos(ctheta)*math.cos(theta[0,0]))*math.sin(theta[3,0])*math.cos(theta[1,0]) ],
                        [-0.1588*math.cos(theta[3,0])],
                        [0],
                        [0],
                        [0]])
    Final6 = np.array([[0],
...
def pose_EE(transformation_matrix):  
    ...
    p = PoseStamped()
    # p.header.frame_id = "turtlebot/swiftpro/manipulator_base_link"       
...
def goal_pose(trans, angle):  
    ...
    g_pose = PoseStamped()
    # g_pose.header.frame_id = "turtlebot/swiftpro/manipulator_base_link"     
