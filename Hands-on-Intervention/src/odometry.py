...
class DifferentialDrive:
    ...
    def joint_state_callback(self, msg):
        ...
        # if msg.name[0] == "turtlebot/kobuki/wheel_encoder_left":
        if msg.name[0] == "turtlebot/kobuki/wheel_left_joint":
            ...
            if (self.left_wheel_received):
                ...
                self.Xk[0,0] = self.Xk[0,0] + np.cos(self.Xk[2,0]) * self.v * dt 
                self.Xk[1,0] = self.Xk[1,0] + np.sin(self.Xk[2,0]) * self.v * dt   
                self.Xk[2,0] = self.Xk[2,0] + self.w * dt          
                print('position', self.Xk)   
                            
                Ak = np.array([[1, 0, -np.sin(self.Xk[2,0])*self.v*dt], 
                                [0, 1, np.cos(self.Xk[2,0])*self.v*dt],
                                [0, 0, 1]])    

                Bk = np.array([[np.cos(self.Xk[2,0])*dt*0.5, np.cos(self.Xk[2,0])*dt*0.5],    
                                    [np.sin(self.Xk[2,0])*dt*0.5, np.sin(self.Xk[2,0])*dt*0.5],  
